...
def listening_evaluation():
    ...
    for i in range(4):
        ...
        with open(f"judge_system/output/listening/part{i+1}.txt", "w", encoding="utf-8") as out_f:
            out_f.write(str(task))

        with open("judge_system/evaluation_prompts/listening.txt", "r", encoding="utf-8") as f:
            eval_prompt = f.read()
            ...
            with open(f"judge_system/output/listening/part{i+1}_llm_feedback.txt", "w", encoding="utf-8") as out_f:
